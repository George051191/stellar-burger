...
export const RegistrationPage: FunctionComponent = () => {
  ...
  const { userEmail, isUserSent, userName } = useSelector(
  ...
  const newUserCreating = (evt: React.FormEvent<HTMLFormElement>) => {
    evt.preventDefault();
    dispatch(
      createUser(emailInputValue, passwordInputValue, nameInputValue, token)
    );
    setNameInput("");
    setEmailInput("");
    setPasswordInput("");
    ...
    <div className={styles.pagestyle}>
      <form className={styles.form} onSubmit={(evt) => newUserCreating(evt)}>
        <fieldset className={styles.inputsfield}>
          <h2 className={`text text_type_main-medium ${styles.header}`}>
            Регистрация
          </h2>
          <Input
            type="text"
        ...
        <Button type="primary" size="medium">
          {isUserSent ? "Запрос отправлен" : "Зарегистрироваться"}
        </Button>
        <div className={`${styles.redirectmenu}  mt-20`}>
          <p
            className={`${styles.registration} text text_type_main-default text_color_inactive `}
          >
