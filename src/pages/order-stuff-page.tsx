...
export const OrderStuffPage: FunctionComponent = () => {
  ...
  const currentItems: TIngredient[] = ingredients.filter((item) => {
    return (
      currentFeedOrder && currentFeedOrder.ingredients.indexOf(item._id) > -1
    );
  });

  const totalCost =
    currentItems &&
    calculateCost(currentItems.slice(1), currentItems[0]?.price);
  ...
  return (
    <div className={styles.conteiner}>
      <p className={` text text_type_digits-default mb-10 ${styles.idstyle}`}>
        #5545454
      </p>
