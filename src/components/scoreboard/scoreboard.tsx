...
export const ScoreBoard: FunctionComponent = () => {
  ...
  const doneOrders: TFeedOrder[] | null =
    ordersData &&
    ordersData.orders.filter((item) => {
      return item.status === "done";
    });

  const ordersInProcess: TFeedOrder[] | null =
    ordersData &&
    ordersData.orders.filter((item) => {
        ...
        <div className={styles.box}>
          ...
          <div className={styles.ulbox}>
            {doneOrders?.map((item, index) => (
              <p
                key={index}
                className={`text text_type_main-default ${styles.statusdone}`}
              >
                {item.number}
              </p>
            ))}
