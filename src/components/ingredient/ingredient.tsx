  ...
  return (
    ...
    >
      <li
        ref={dragRef}
        id={props.id}
        className={cardStyles.ingredient__item}
        onClick={(evt) => {
          dispatch({
            type: CLICK_ON_INGREDIENT,
            item: selectIngredient(evt, ingredients),
          });
        }}
      >
        {!!props.amount && !!elements.length && (
          <Counter count={props.amount} size="default" />
        )}
        {!!bun.amount && props.type === "bun" && bun._id === id && !!bun && (
          <Counter count={bun.amount} size="default" />
        )}
        <img
          className={`mb-1 pl-4 pr-4  ${cardStyles.ingredient__image}`}
          src={props.image}
          alt={props.name}
        />
        ...
        <p
          className={`mt-1 text text_type_main-default ${cardStyles.ingredient__text}`}
        >
          {props.name}
        </p>
      </li>
