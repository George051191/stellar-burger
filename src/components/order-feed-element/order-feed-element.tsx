...
export const OrderFeedElement: FunctionComponent<IOrderFeedElement> = (
  props
) => {
  ...
  const style =
    props.status === "done"
      ? `text text_type_main-default  ${styles.textgreen}`
      : `text text_type_main-default  ${styles.textwhite}`;

  return (
    <Link
      className={styles.link}
      to={{ pathname: props.path, state: { background: location } }}
      onClick={() => {
        dispatch({ type: CLICK_ON_ORDER, payload: props.number });
      }}
    >
        ...
        <div className={`${styles.conteiner} mb-6`}>
          <div className={`${styles.imagesbox} mr-6`}>
            {currentItems.slice(0, 6).map((item, index) => (
              <IngredientImage image={item.image_mobile} key={index} />
            ))}
