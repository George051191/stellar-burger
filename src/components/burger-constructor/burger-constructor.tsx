...
export const BurgerConstructor: FunctionComponent = () => {
  ...
  const { orderButtonIsClicked, requestIsSuccessed, orderNumber } = useSelector(
  ...
  const itemRemove = React.useCallback(
    (itemKey: number, itemId: string) => {
  ...
  const orderDetailsRequestSending = () => {
    const idArray = elements.map((item) => {
      return item._id;
    });
    dispatch({ type: OPEN_ORDER_POPUP });
    dispatch(
      getOrderNumber([...idArray, bun._id], token, refresh, refreshMainToken)
    );
  };
  ...
  const style = isDrag
    ? consructorStyles.burgerconstructor__dropconteiner
    : consructorStyles.burgerconstructor__conteiner;
  const cartStyle =
    elements.length >= 6
      ? consructorStyles.burgerconstructor__elementswithscroll
      : consructorStyles.burgerconstructor__elements;
  const emptyBurgerStyles = !calculateCost(elements, bun.price)
    ? consructorStyles.burgerbox
    : consructorStyles.burgerboxinvisible;
  ...
  return (
    <div ref={dropTarget} className={`pl-4   ml-10 pt-25 ${style}`}>
          ...
          .map((item, index) => (
            <ConstructorItem id={item.uid} index={index} key={item.uid}>
              <ConstructorElement
                ...
                thumbnail={item.image_mobile}
                price={item.price}
                handleClose={() => itemRemove(item.uid, item._id)}
              />
            </ConstructorItem>
          ))}
      </ul>
      ...
      <OrderRegistration
        ...
        styles={`mt-10 ${consructorStyles.burgerconstructor__cost}`}
        cost={calculateCost(elements, bun.price)}
      />
      {orderButtonIsClicked && requestIsSuccessed && (
        <Modal
          closeModal={orderPopupClose}
          modalHeaderStyles={consructorStyles.burgerconstructor__modalheader}
        >
